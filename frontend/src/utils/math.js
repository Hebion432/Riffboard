...
export const isPointCloseToLine = (x1, y1, x2, y2, pointX, pointY) => {
  // basically triangle ke teeno side nikaal rahe hai ( as an external point will make the line a triangle)
  ...
  return Math.abs(distToStart + distToEnd - distLine) < ELEMENT_ERASE_THRESHOLD; // agar eraser ka size badhayenge then threshold ka size bhi badha denge ( min it can be 0 which is when the point is on the line )
};
